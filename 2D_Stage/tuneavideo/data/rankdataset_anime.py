...
class TuneAnimeMVImageDataset(Dataset):
    def __init__(
        ...
        # self.image_root_path = "/mnt/pfs/users/zhangjiapeng/workspace_for_IP_Adapter/rgb_path"
        self.image_root_path = "/mnt/pfs/users/zhangjiapeng/workspace_for_IP_Adapter/render_image"
    ...
    def process_image(self, image_dir, use_id, crop=False):
        # image
        image_path = os.path.join(self.image_root_path, image_dir  + "_a_pose", image_dir + "_" + use_id + "_rgb.png")
        # pose_path = os.path.join(self.image_root_path, image_dir + "_a_pose", image_dir+"_"+use_id+"_pose.png").replace("rgb_path", "new_render")
        ...
        if crop:
            ...
            crop_area = (128, 0, 640, 768)
            pose_image = self.totensor(np.array(pose_image.crop(crop_area)).astype(np.float32)) / 255.
        else:
    ...
    def __getitem__(self, index):
        ...
        fn_list = glob(os.path.join(self.image_root_path, image_dir, "*.png"))
        inp_index = np.random.randint(0, len(fn_list))
        image_inp = fn_list[inp_index]
        ...
        # image_inp = np.asarray(Image.open(os.path.join(self.image_root_path, image_dir, image_dir + "_" + image_inp + "_rgb.png"))) / 255.
        image_inp = np.asarray(Image.open(image_inp)) / 255.
        bg_color = self.get_bg_color(self.bg_color)
        alpha = image_inp[..., 3:4]
        image_inp = image_inp[..., :3] * alpha + bg_color * (1 - alpha)
        image_inp = self.totensor(image_inp)
        
        assert self.num_views == 4, "only 4 views support for now (instant3d)"
        select_id = 0
        # if self.valid:
        #     select_id = 0
        # else:
        ...
        #         select_id = 0
        #     else:
        #         select_id = self.image_index[np.random.randint(4)]
        for i in range(self.num_views):
            use_id = str(select_id + i * 2).zfill(3)

            # camera_matrix
            camera_vector = torch.tensor(json.load(open(os.path.join(self.image_root_path, image_dir + "_a_pose", image_dir+"_"+use_id+".json")))["extrinsicMatrix"]["elements"]).float()
            camera_matrix = camera_vector.reshape(4,4).transpose(1,0)[:3,:4] # 3 * 4
            camera_matrixs.append(camera_matrix)
            if self.use_modal == 'rgb':
                image, pose_image = self.process_image(image_dir, use_id, crop=False)
