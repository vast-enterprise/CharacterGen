...
class TuneAnimeMVImageDataset(Dataset):
    def __init__(
        ...
        if valid:
            ...
            prompt_path = os.path.join(self.image_root_path, "imagedream.jsonl")
    ...
    def __getitem__(self, index):
        ...
        image_inp = np.asarray(Image.open(os.path.join(self.image_root_path, image_dir, "grid.png"))) / 255.
        # bg_color = self.get_bg_color(self.bg_color)
        # alpha = image_inp[..., 3:4]
        # image_inp = image_inp[..., :3] * alpha + bg_color * (1 - alpha)
        image_inp = self.totensor(image_inp)
        imgs = np.asarray(Image.open(os.path.join(self.image_root_path, image_dir + "_a_pose", "grid_normal.png")))
        imgs = (imgs / 127.5 - 1.0).astype(np.float32)
        imgs = imgs.astype(np.float32)*2.0-1.0
        imgs = self.totensor(imgs)
        example = {
            "conditioning_pixel_values": image_inp,
            "pixel_values": imgs,
        }
        return example